...
1. 前置背景：App 为 Hybrid 架构的 H5 项目，路由采用多路由方案，缓存使用了 Pinia 和持久化插件，通过持久化插件在多个页面之间共享数据
...
bug 定位的过程本身是一个从问题表现收集信息，基于已有的信息和经验，提出可能的原因假设，再通过各种各种方法(如日志、断点调试等)验证假设，再基于测试人员或者自身实践的反馈去验证问题是否得到解决的不断迭代过程，这里推荐一篇组内大佬对[bug 定位方法论总结的文章](https://blog.litong.life/frames-of-mind/)。

<img src="/20.png" style="zoom:33%;" />

(PS：图片来源组内大佬[文章](https://blog.litong.life/frames-of-mind/))
...
由于开发环境无法复现该问题，只能交由香港测试人员验证，验证结果是传入的 info 值是正常的。并且在第一轮验证后，测试人员还反馈其他业务模块也在同一台机器上出现了类似的白屏问题。这一轮得出的结论是组件业务代码是没有问题的。
...
这里先简单介绍下我们项目内的缓存方案，我们项目是 Hybrid 架构的多页面路由 H5 项目，在多个页面之间需要共享数据。使用的是 Pinia 进行全局数据管理，通过`pinia-plugin-persistedstate`持久化插件自动将 Pinia 数据同步进 localStorage 内。新页面会用 localStorage 内的数据对 Pinia 进行初始化，实现数据共享。
...
测试人员反馈在 B 页面发生白屏时，跳转前的 A 页面并没有弹出系统提示。并且在收集白屏机器的信息时又收集到了一个重要信息。在 B 页面通过 vConsole 查看 localStorage 内的值，发现在 A 页面初始化请求接口时存入的 list 是有的，唯一丢失的是跳转前存入的 state。
...
在开发环境写完测试代码后测试后发现 console 打印出来的 localStorage 值是异常的，即获取到的 localStorage 内没有刚刚存入的 state 字段。但是通过 safari 的 devTools 内查看最后是有 state 字段的。即在更改 Pinia 缓存后，同步获取 localStorage 内的值是未更新前的，但最后 localStorage 内是会被更新的，因此可以得出初步猜测是`pinia-plugin-persistedstate`持久化插件的数据写入并不是同步的过程
...
尝试手动同步不依赖于持久化插件的自动同步，直接在设置 Pinia 缓存的 setPageCache 方法内调用持久化插件暴露的$persist 方法，直接进行手动同步，以确保在 Pinia 数据更新后，能立即写入最新数据到本地缓存中，伪代码如下。
...
在开发环境测试后，发现通过手动调用$persist 函数后，在路由跳转前能够直接获取到更新后的 localStorage 值了，但在香港测试人员验证后，反馈依然还是存在白屏问题，并且在 B 页面打印出的 localStorage 内没有存入的 state
...
在开发环境和生产环境测试后，从 vConsole 得到的反馈是一次 Pinia 更改，会调用两次 setItem。手动同步和自动同步写入的数据是一致的，但自动同步的调用时机在手动同步之后。
...
nextTick 无法解决白屏问题，但设置 setTimeout 后，白屏问题消失，并且定时器的延时需要在 500ms 以上。虽然不知道为什么定时器可以解决这个问题，但抱着先把 Bug 处理掉的心态，我们在所有反馈出现过白屏的地方，都添加了 500ms 的延迟跳转。
...
1. A 页面最后一次调用 localStorage.setItem，传入的数据符合预期

   <img src="/13.png" style="zoom:33%;" />

2. B 页面第一次调用 localStorage.getItem，读取到的数据不符合预期
...
从日志可以看出 A 页面一共调用了四次 setItem：store_1723535995666_9，store_1723535995667_10，CONTACT_1723535996658_11，CONTACT_1723535996663_12。只有 11 和 12 是有 state 的，即在点击事件内产生的 setItem 才会有相关数据，而页面初始化过程产生的 9 和 10 两次写入是没有 state 字段的。在 B 页面初始化时读取到的是 id 为 store_1723535995667_10 的数据。即 B 页面读取的并不是 A 页面最后一次写入的数据，验证了我们的猜想
...
在进入 B 页面后，change 事件没有被触发，轮询获取到的也不是 A 页面最后存入的值，日志如下
...
返回 A 页面后，读取到的数据 id 为 12，即 A 页面最后一次写入的数据，从日志结果可以得出结论，当 A 页面退出栈顶后，localStorage 的同步会被中止，只有当 A 页面重新进入栈顶后，才会继续同步 localStorage。
...
该问题的本质原因其实是 iOS 特定版本系统 IO 的问题，无法在 h5 层面进行解决，因此只能更换持久化存储的方案，将 localStorage 替换为 app 提供的 app storage。
